...
class MyCV2(object):
    ...
    def cat_mosaic(self):
        ...
        cv2.imwrite(f'{vo.context}cat_mosaic.png', mos)
        cv2.waitKey(0)  # 키 입력을 기다리는 대기 함수, 0은 즉시 실행
    ...
    def lena(self):
        ...
        original = cv2.imread(lena, cv2.cv2.IMREAD_COLOR)
        gray = cv2.imread(lena, cv2.cv2.IMREAD_GRAYSCALE)
        unchanged = cv2.imread(lena, cv2.cv2.IMREAD_UNCHANGED)
